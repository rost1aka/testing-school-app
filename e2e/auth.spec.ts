...
test("the user's name in the header leads to the profile, addresses and all", async ({ page }) => {
  ...
  // Loaded fresh, because the header only learns about the new session when
  // the session provider mounts: signing in leaves the client-side session
  // untouched until the next full page load.
  await page.goto("/");
